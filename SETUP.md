...
eBay Finder works **with no key at all** — it builds expert eBay search URLs and
...
`~/.cache/ebay_finder/token.json` (override with `EBAY_TOKEN_CACHE`).

## Limits & etiquette
- The client-credentials (application) tier is free, ~1,000–5,000 calls/day — plenty
  for personal hunting.
- eBay Finder uses the **official Buy/Browse API** only. It never scrapes or
